%% Clear
fprintf('Clearing the workspace \n')
clear; 

%% Initial Setup parameters
fprintf('Initializing the parameters \n')
...
fprintf('Reading the image stack and stretching the histogram \n')
% Read the stack
if brk_stck
...
fprintf('Correct the line-offset due to bi-directional scanning \n')
if do_scn_phs_crcn
    %     nFrames = 200;
    nFrames = phs_crcn_frm; % Phase correction number of frames
    bidir_us = 5; % up-sampling to reach a sub-pixel image registration
...
%% Smoothing the data
fprintf('Smoothing the image histogram scaling \n')
...
% im_stck = im2uint16(im_stck); %

clear high_in hist_cnt hist_cnt_low_ix hist_lctn hist_mx_ix low_in
%% set parameters for motion correction (first try out rigid motion correction)
fprintf('Setting the motion corrections parameters \n')
if do_mtn_crcn
...
fprintf('Perform motion correction \n')
if do_mtn_crcn
    tic;
...
fprintf('Cell detection parameters \n')
...
%% Clean the selected ROI Centers
fprintf('Clean the selected ROIs in the corners \n')
center_mod = center;
for i=1:K
    if center_mod(i, 1) > 490 ...
            || center_mod(i, 1) < 22 || center_mod(i, 2) > 490 ...
            || center_mod(i, 2) < 10
        center_mod(i, :) = 0;
    end
end
[cntr_row, ~] = find(center_mod(:,1) == 0);
...
clear i center_mod cntr_row
%% manually refine components (optional)
fprintf('Manually refine the ROIs \n')
...
fprintf('Update the spatial components \n')
Yr = reshape(im_stck,d,T);
[A,b,Cin] = update_spatial_components(Yr,Cin,fin,[Ain,bin],P,options);

%% update temporal components
fprintf('Update the temporal components \n')
P.p = 0;    % set AR temporarily to zero for speed
[C,f,P,S,YrA] = update_temporal_components(Yr,A,b,Cin,fin,P,options);

%% classify components
fprintf('Classify components \n')
tic; 
...
fprintf('Classify component 2nd stage \n')
...
fprintf('Display the kept components \n')
A_keep = A(:,keep);
C_keep = C(keep,:);
figure;
subplot(121); 
...
fprintf('Merge components \n')
[Am,Cm,K_m,merged_ROIs,Pm,Sm] = merge_components(Yr,A_keep,b,C_keep,f,P,S,options);

%% Plotting the merged components 
fprintf('Plotting merged components \n')
...
fprintf('Excluding the rejected components \n')
...
%% do some plotting
fprintf('Plotting the detected cell units \n')
[A_or,C_or,S_or,P_or] = order_ROIs(A2,C2,S2,P2); % order components

badCellsIndexed = zeros(size(A_or, 2),1);
for i=1:size(A_or, 2)
    if size(find(A_or(:, i)), 1) < 50 ...
            || size(find(A_or(:, i)), 1) > 900
        badCellsIndexed(i) = 1;
    end
end
[badCellsRow, ~] = find(badCellsIndexed);
A_or(:, badCellsRow) = [];
C_or(badCellsRow, :) = [];
S_or(badCellsRow, :) = [];
P_or.gn(badCellsRow) = [];
...
%% WS Cleanup
fprintf('Cleaning up the workspace before saving \n')
if (clr_ws)
   clearvars A A2 A_keep Ain Am ans  b badCellsIndexed badCellsRow ...
       bg_cmpt bidir_us bin C C2 ca_prfl C_keep ...
